...
`treeland-inspector` is an extensible C++ command-line inspector for Treeland
...
```bash
cmake -S . -B build -DBUILD_TREELAND_INSPECTOR=ON
cmake --build build --target treeland-inspector
sudo cmake --install build --component treeland-inspector
```

The executable is installed to `/usr/local/bin/treeland-inspector` by default.
...
```bash
sudo -u dde -- /usr/local/bin/treeland-inspector --tree
```
...
```bash
sudo -u dde -- /usr/local/bin/treeland-inspector --cursor
```
