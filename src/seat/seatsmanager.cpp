...
WSeat *SeatsManager::createSeat(const QString &name, bool isFallback)
{
    ...
    // order (~WServer runs as a sibling before ~SeatsManager's member
    // teardown), so pruning from the hook is safe there; ~SeatsManager
...
void SeatsManager::unhookSeat(WSeat *seat)
{
