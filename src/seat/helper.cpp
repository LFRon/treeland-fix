...
void Helper::handleRequestDragForSeat(WSeat *seat, WSurface *)
{
    ...
    QObject::connect(qwDrag, &qw_drag::before_destroy, this, [seat, drag] {
        drag->data = NULL;
        seat->setAlwaysUpdateHoverTarget(false);
    });

    // TODO: https://gitlab.freedesktop.org/wayland/wayland/-/work_items/444
    const auto updateCursor = [seat, drag] {
        if (!seat->cursor() || drag->grab_type != WLR_DRAG_GRAB_KEYBOARD_POINTER)
            return;
