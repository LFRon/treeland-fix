...
void Helper::init(Treeland::Treeland *treeland)
{
    ...
    m_xwaylandOutputManager->setFilter([](WClient *client) { return isXWaylandClient(client); });
    ...
    connect(m_renderWindow, &WOutputRenderWindow::effectiveDevicePixelRatioChanged,
            this, [this](qreal dpr) {
        if (m_xwaylandOutputManager)
            m_xwaylandOutputManager->setScaleOverride(dpr);
    });
