...
void Helper::onOutputAdded(WOutput *output)
{
    ...
    o->enable();
    if (m_scaned && m_mode == OutputMode::Copy) {
    ...
    if (m_scaned && m_mode == OutputMode::Extension) {
        const QString addedOutputId = WallpaperManager::getOutputId(o);
    ...
    const bool shouldDisableOutput = !m_scaned;
...
void Helper::onOutputTestOrApply(qw_output_configuration_v1 *config, bool onlyTest)
{
    ...
    if (m_scaned && m_outputManagerHelper && m_globalConfig && !m_globalConfig->singleOutputId().isEmpty()) {
        const QString singleOutputId = m_globalConfig->singleOutputId();
        for (const auto &state : std::as_const(states)) {
            if (state.enabled && !state.output->isEnabled()) {
                ...
                if (output && WallpaperManager::getOutputId(output) != singleOutputId) {
                    qWarning() << "xyb---onOutputTestOrApply clearSingleOutputConfig";
    ...
    for (const auto &state : std::as_const(states)) {
        qWarning() << "xyb--output state"
                   << state.output->name();
        // Skip outputs that have been removed (e.g., disabled in Copy mode)
        Output *output = getOutput(state.output);
        if (!output) {
            qWarning() << "xyb----Skip outputs that have been removed";
            continue;
        }
        ...
        qWarning() << "xyb----output name" << outputName;
        outputHelper->scheduleCommitJob(
            [self, config, extraState, renderWindow, viewport, outputName, enabled](bool success, WOutputHelper::ExtraState committedState) {
                ...
                qWarning() << "xyb--Output configuration commit finished"
                                      << outputName
                                      << "enabled:" << enabled
                                      << "success:" << success;
...
bool Helper::restoreConfiguredCopyMode()
{
    ...
    /*qCInfo(lcTlOutput)*/ qWarning() << "Restoring configured Copy Mode"
...
void Helper::restoreInitialOutputConfiguration()
{
    runWhenTreelandConfigInitialized(m_globalConfig.get(), this, [this] {
        const QString singleOutputId = m_globalConfig->singleOutputId();
        qWarning() << "xyb-----singleOutputId:" << singleOutputId;
        if (!singleOutputId.isEmpty()) {
            if (findOutputById(singleOutputId)) {
                qWarning() << "xyb-----restoreConfiguredSingleOutput";
                m_outputManagerHelper->restoreConfiguredSingleOutput(getWorkspaceSurfaces(), true);
                return;
            }

            qWarning() << "xyb-----restoreExtensionModeFromConfig";
            m_outputManagerHelper->clearSingleOutputConfig();
            restoreExtensionModeFromConfig();
            return;
        }

        qWarning() << "xyb-----createCopyOutput:" << m_globalConfig->createCopyOutput();
...
void Helper::enableAllOutput()
{
    for (auto *output : std::as_const(m_outputList)) {
        ...
        wlr_output_commit_state(output->output()->nativeHandle(), &state);
