...
namespace {
constexpr auto remoteObjectName = "TreelandRemote";
constexpr auto remoteObjectSocketName = "org.deepin.dde.treeland.qro";
}

class DDMRemoteObjectV1Private : public TreelandRemoteSource
{
public:
...
DDMRemoteObjectV1Private::DDMRemoteObjectV1Private(DDMRemoteObjectV1 *qObject)
    : TreelandRemoteSource()
    , q(qObject)
{
    auto *greeterProxy = Helper::instance()->greeterProxy();
    QObject::connect(greeterProxy, &GreeterProxy::lockChanged, this, [this](bool isLocked) {
        Q_EMIT lockStateChanged(isLocked);
    });
}
...
{
    qCInfo(treelandCore) << "DDM remote: switchToGreeter";
...
void DDMRemoteObjectV1Private::lock()
{
    ...
    Helper::instance()->greeterProxy()->setLock(true);
}
