...
void RootSurfaceContainer::detectEdgeTilingForSeat(WSeat *seat)
{
    ...
    if (mrState.surface && mrState.edges == Qt::Edges()) {
        ...
        if (out) {
            ...
            // Side tiling takes priority over top maximize (matches KWin):
            // the top-left corner triggers Left, not Maximize.
    ...
    // Only re-evaluate activation when the edge state actually changes.
    if (mrState.detectedTileMode != mode || mrState.edgeTileInnerBorder != innerBorder) {
        ...
        if (mode == QuickTile::Mode::None) {
            // Left the edge zone: stop timer, hide preview.
            container->stopEdgeTileDelay();
            mrState.edgeTilePreviewActive = false;
            updateEdgeTilePreview(QuickTile::Mode::None, nullptr);
        } else if (innerBorder) {
            // Inner edge: don't activate yet. Start (or restart) the 250ms
            // delay timer; hide any stale preview while waiting.
            container->stopEdgeTileDelay();
            mrState.edgeTilePreviewActive = false;
            updateEdgeTilePreview(QuickTile::Mode::None, nullptr);
            container->startEdgeTileDelay();
        } else {
            // Outer edge: activate immediately.
            container->stopEdgeTileDelay();
...
void RootSurfaceContainer::endMoveResizeForSeat(WSeat *seat)
{
    ...
    // Hide the edge-tiling preview once the drag ends (applied or not).
    updateEdgeTilePreview(QuickTile::Mode::None, nullptr);
}
...
void RootSurfaceContainer::cancelMoveResizeForSeat(WSeat *seat)
{
    ...
    // Hide the edge-tiling preview when the drag is cancelled.
    updateEdgeTilePreview(QuickTile::Mode::None, nullptr);
}
