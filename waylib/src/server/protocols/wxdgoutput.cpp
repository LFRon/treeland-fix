...
static void output_update(struct way_xdg_output_v1 *xdg_output) {
    ...
    if (xdg_output->manager->output_scale > 0.0) {
        x = static_cast<int32_t>(std::floor(layout_output->x * xdg_output->manager->output_scale));
        y = static_cast<int32_t>(std::floor(layout_output->y * xdg_output->manager->output_scale));
    } else {
        x = layout_output->x;
        y = layout_output->y;
    }
    ...
    if (xdg_output->manager->output_scale > 0.0) {
        width = static_cast<int>(std::ceil(width * xdg_output->manager->output_scale));
        height = static_cast<int>(std::ceil(height * xdg_output->manager->output_scale));
    }
