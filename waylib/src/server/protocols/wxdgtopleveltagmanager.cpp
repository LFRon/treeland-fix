...
static void manager_handle_set_tag(struct wl_client *client,
...
static void manager_handle_set_description(struct wl_client *client,
                                           struct wl_resource *manager_resource,
                                           struct wl_resource *toplevel_resource,
                                           const char *description)
{
...
static WXdgToplevelTagManagerV1 *XDG_TOPLEVEL_TAG_MANAGER = nullptr;
...
public:
    ...
    static void handle_set_tag(struct wl_listener *listener, void *data)
    {
    ...
    static void handle_set_description(struct wl_listener *listener, void *data)
    {
...
{
    if (XDG_TOPLEVEL_TAG_MANAGER) {
        qFatal("There are multiple instances of WQuickXdgToplevelManger");
    }

    XDG_TOPLEVEL_TAG_MANAGER = this;
}
...
void WXdgToplevelTagManagerV1::create([[maybe_unused]] WServer *wserver)
{
    ...
    d->manager = way_xdg_toplevel_tag_manager_v1_create(*server()->handle(), 1);
    m_handle = d->manager;
    ...
    d->set_description_listener.notify = WXdgToplevelTagManagerV1Private::handle_set_description;
