...
public:
    struct Q_DECL_HIDDEN LayerData {
        LayerData(OutputLayer *l, wlr_output_layer *layer)
            : layer(l)
            , wlrLayer(layer)
        ...
        ~LayerData() {
            ...
            if (wlrLayer) {
                wlr_output_layer_destroy(wlrLayer);
                wlrLayer = nullptr;
            }
        }

        OutputLayer *layer;
        wlr_output_layer * wlrLayer;
    ...
    inline wlr_output *output() const {
        return outputViewport()->output()->handle();
    }
    ...
    inline WOutputViewport *outputViewport() const {
        return m_output;
    }
    ...
    void deferHardwareCursorCommit(wlr_buffer *buffer, const QPoint &hotSpot);
    void flushPendingHardwareCursor();
    void cancelPendingHardwareCursor();
...
private:
    ...
    // Vulkan only: the cursor buffer rendered into the current frame's command
    // buffer. set_cursor() must wait until that command buffer has been
    // submitted and completed, or the cursor plane can scan out content that
    // the GPU has not rendered yet.
    wlr_buffer *m_pendingCursorBuffer = nullptr;
    QPoint m_pendingCursorHotSpot;
    // Set when a deferred set_cursor() failed, so later frames keep the cursor
    // on the software canvas path until the cursor content is re-rendered.
    // Retrying the same buffer every frame would otherwise busy-loop the
    // compositor's render loop.
    bool m_hardwareCursorCommitBroken = false;
...
public:
    WOutputRenderWindowPrivate(WOutputRenderWindow *)
        : QQuickWindowPrivate()
        , listenerOwner(std::make_unique<WListenerOwner>())
    {
    }
    ...
    inline bool containsOutput(WOutput *o) const {
        for (auto output : std::as_const(outputs)) {
            if (output->outputViewport()->output() == o)
                return true;
        }
        return false;
    }
    ...
    doRenderOutputs(wlr_output *needsFrameOutput, const QList<OutputHelper *> &outputs,
                    bool forceRender);
    ...
    void doRender(wlr_output *needsFrameOutput, const QList<OutputHelper*> &outputs,
    ...
    // Consecutive QRhi beginOffscreenFrame failures without a completed frame.
    // A single Error (not device-lost) failure only skips the frame; repeated
    // failures freeze resource retirement forever, so escalate to fatal.
    int consecutiveBeginFrameFailures = 0;
    ...
    // them; reset early from ~WOutputRenderWindow.
    std::unique_ptr<WListenerOwner> listenerOwner;
};
...
bool OutputHelper::attachLayer(OutputLayer *layer)
{
    ...
    // wlr_output will destory this layer on wlr_output destroy
    auto qwlayer = wlr_output_layer_create(output());
    if (!qwlayer)
        return false;
...
wlr_buffer *OutputHelper::renderLayer(LayerData *layer, bool *dontEndRenderAndReturnNeedsEndRender)
{
...

    {
        ...
        mapRect = mapRect & QRectF(QPointF(0, 0), outputViewport()->size());
        ...
        if (buffer) {
            ...
            if (dontEndRenderAndReturnNeedsEndRender) {
                *dontEndRenderAndReturnNeedsEndRender = true;
                ...
                if (vulkanRenderer
                    && !renderWindowD()->releaseRenderBuffer(
...
WBufferRenderer *OutputHelper::afterRender()
{
    ...
    for (LayerData *i : std::as_const(m_layers)) {
        ...
        layers.append({
            ...
            .damage = &i->renderer->damageRing()->current,
            .accepted = false
        });
...

    {
        ...
        if ((!outputViewport()->disableHardwareLayers() || topLayer->layer->forceLayer())
            && !(ok && layers.last().accepted)
            && (topLayer->layer->layer->flags() & WOutputLayer::Cursor)) {
            if (tryToHardwareCursor(topLayer)) {
                Q_ASSERT(topLayer->renderer->lastBuffer() == layers.last().buffer);
                Q_ASSERT(!hasHardwareCursor);
                hasHardwareCursor = true;
                bool ok = topLayer->layer->accept(outputViewport(), true);
                Q_ASSERT(ok);
    ...
    for (int i = layers.length() - 1; i >= 0; --i) {
        ...
        if (needsSoftwareCompositeEndIndex == -1
            && outputViewport()->disableHardwareLayers()
            && !layer->forceLayer()) {
        ...
        if (needsSoftwareCompositeEndIndex == -1) {
            if (ok && state.accepted) {
                bool ok = layer->accept(outputViewport(), true);
                Q_ASSERT(ok);
                continue;
            } else {
                needsSoftwareCompositeEndIndex = i;
        ...
        if (layer->forceLayer() || layer->keepLayer()
            ...
            bool ok = layer->accept(outputViewport(), false);
            Q_ASSERT(ok);
            if (layer->forceLayer())
                forceShadowRender = true;
            needsSoftwareCompositeBeginIndex = i;
        } else if (!outputViewport()->ignoreSoftwareLayers()) {
            bool ok = layer->reject(outputViewport());
            Q_ASSERT(ok);
        }
    }
    ...
    if (needsCompositeLayers.isEmpty()
        ...
        || outputViewport()->ignoreSoftwareLayers()) {
...
WBufferRenderer *OutputHelper::compositeLayers(const QList<LayerData*> layers, bool forceShadowRenderer)
{
    ...
    const bool isVulkan = WRenderHelper::getGraphicsApi(renderWindowD()->rc())
                          == QSGRendererInterface::Vulkan;
    const bool needsShadowRenderer = forceShadowRenderer
                                     || !bufferRenderer()->isColorPreserved();
    const bool usingShadowRenderer = needsShadowRenderer && !isVulkan;
    if (needsShadowRenderer && !usingShadowRenderer) {
...
bool OutputHelper::tryToHardwareCursor(const LayerData *layer)
{
    ...
    do {
        ...
        auto buffer = layer && layer->renderer->lastBuffer()
                          ...
                          : nullptr;
        if (!buffer) {
            cancelPendingHardwareCursor();
            if (!m_hardwareCursorRenderComplete)
                return true;

            if (set_cursor)
                set_cursor(output(), buffer, 0, 0);
        ...
        auto move_cursor = output()->impl->move_cursor;

        if (!set_cursor || !move_cursor)
            break;

        QSize pixelSize = QSize(buffer->width, buffer->height);
        auto get_cursor_sizes = output()->impl->get_cursor_sizes;
        auto get_cursor_formsts = output()->impl->get_cursor_formats;
        ...
        if (get_cursor_sizes) {
            if (Q_UNLIKELY(lcWlRenderer().isDebugEnabled()))
                qCDebug(lcWlRenderer) << "Supported cursor sizes for output" << outputViewport() << ":";

            bool foundTargetSize = false;
            size_t sizes_len = 0;
            const auto sizes = get_cursor_sizes(output(), &sizes_len);
        ...
        if (needsRepaintCursor) {
            ...
            if (m_cursorDirty || !newBuffer ||
                ...
                // The cursor content was re-rendered into a fresh buffer, so
                // a previously failed deferred set_cursor() is worth one
                // retry with the new buffer.
                if (newBuffer && isVulkan)
                    m_hardwareCursorCommitBroken = false;
            }
        ...
        if (isVulkan) {
            if (m_hardwareCursorCommitBroken) {
                // A previous deferred set_cursor() failed for this output;
                // keep the cursor on the software canvas path until the cursor
                // content is re-rendered.
                break;
            }
            // The buffer content is produced by the current frame's command
            // buffer, which is only submitted and awaited at endFrame().
            // Defer set_cursor() until after that point so the cursor plane
            // can never scan out content the GPU has not rendered yet.
            deferHardwareCursorCommit(buffer, hotSpot);
            m_hardwareCursorRenderComplete = true;
        } else if (!set_cursor(output(), buffer, hotSpot.x(), hotSpot.y())) {
            qCWarning(lcWlCursor)
        ...
        const auto outputSize = outputViewport()->output()->size();
        ...
        if (!move_cursor(output(), cleanTransform.x, cleanTransform.y)) {
            ...
            // The caller will composite the cursor into the canvas instead, so
            // a deferred set_cursor() for this frame must not run afterwards.
            cancelPendingHardwareCursor();
            break;
        }
...
void OutputHelper::deferHardwareCursorCommit(wlr_buffer *buffer, const QPoint &hotSpot)
{
    Q_ASSERT(buffer);
    m_pendingCursorBuffer = buffer;
    m_pendingCursorHotSpot = hotSpot;
}

void OutputHelper::cancelPendingHardwareCursor()
{
    m_pendingCursorBuffer = nullptr;
    m_pendingCursorHotSpot = QPoint();
}

void OutputHelper::flushPendingHardwareCursor()
{
    if (!m_pendingCursorBuffer)
        return;

    auto set_cursor = output()->impl->set_cursor;
    wlr_buffer *buffer = m_pendingCursorBuffer;
    const QPoint hotSpot = m_pendingCursorHotSpot;
    cancelPendingHardwareCursor();
    ...
    if (!set_cursor(output(), buffer, hotSpot.x(), hotSpot.y())) {
        qCWarning(lcWlCursor)
            << "Deferred wlr output set_cursor failed; retrying via a new frame with the software cursor"
            << "output" << output()
            << "bufferSize" << QSize(buffer->width, buffer->height)
            << "hotSpot" << hotSpot;
        if (set_cursor)
            set_cursor(output(), nullptr, 0, 0);
        m_hardwareCursorRenderComplete = false;
        // Arm the breaker: retrying the same buffer every frame would
        // busy-loop the render loop (tryToHardwareCursor() checks this flag
        // before deferring a commit and falls back to the software canvas
        // instead). It is disarmed when the cursor content is re-rendered.
        m_hardwareCursorCommitBroken = true;
        update();
    }
}
...
int WOutputRenderWindowPrivate::indexOfOutputHelper(const WOutputViewport *output) const
{
    for (int i = 0; i < outputs.size(); ++i) {
        if (outputs.at(i)->outputViewport() == output) {
            return i;
        }
    }
...
void WOutputRenderWindowPrivate::init(OutputHelper *helper)
{
    ...
    for (auto layer : std::as_const(layers)) {
        if (layer->outputs().contains(helper->outputViewport())) {
...
// sanity check for Vulkan
    ...
    if (rhiSupport->rhiBackend() == QRhi::Vulkan) {
        ...
        auto phdev = wlr_vk_renderer_get_physical_device(m_renderer);
    ...
    if (rhiSupport->rhiBackend() == QRhi::OpenGLES2) {
        Q_ASSERT(wlr_renderer_is_gles2(m_renderer));
        auto egl = wlr_gles2_renderer_get_egl(m_renderer);
...
void WOutputRenderWindowPrivate::updateSceneDPR()
{
    ...
    for (auto o : std::as_const(outputs)) {
        if (o->outputViewport()->output()->scale() > maxDPR)
            maxDPR = o->outputViewport()->output()->scale();
    }
...
void WOutputRenderWindowPrivate::sortOutputs()
{
    std::stable_sort(outputs.begin(), outputs.end(),
        ...
        return o2->outputViewport()->depends().contains(o1->outputViewport());
    });
}

QVector<std::pair<OutputHelper*, WBufferRenderer*>>
WOutputRenderWindowPrivate::doRenderOutputs(wlr_output *needsFrameOutput, const QList<OutputHelper*> &outputs,
                                            bool forceRender)
{
    if (WRenderHelper::getGraphicsApi(rc()) != QSGRendererInterface::Vulkan) {
        ...
        for (OutputHelper *helper : std::as_const(outputs)) {
            ...
            Q_ASSERT(helper->outputViewport()->output()->scale()
                     <= helper->outputViewport()->devicePixelRatio());
            ...
            wlr_buffer *buffer = helper->beginRender(
                helper->bufferRenderer(),
                helper->outputViewport()->output()->size(),
                format,
                WBufferRenderer::RedirectOpenGLContextDefaultFrameBufferObject,
                                                                     ...
                                                                     QVector<wlr_texture *> *preparedTextures)
{
    ...
    for (auto provider : activeProviders) {
        ...
        auto providerBuffer = provider->wlrBuffer();
...
void WOutputRenderWindowPrivate::doRender(wlr_output *needsFrameOutput,
                                          ...
                                          bool forceRender, bool doCommit)
{
    ...
    const auto finishFrame = [this, q] (const QList<QPointer<WOutput>> &committedOutputs) {
        ...
        // Drop cursor commits deferred for a frame that did not complete, so
        // they can never outlive the frame whose command buffer produced them.
        for (auto helper : std::as_const(this->outputs)) {
            if (helper)
                helper->cancelPendingHardwareCursor();
        }
        ...
        if (isVulkan) {
            ...
            if (beginResult != QRhi::FrameOpSuccess) {
                if (beginResult == QRhi::FrameOpDeviceLost) {
                    ...
                    if (consecutiveBeginFrameFailures >= FatalBeginFrameFailureThreshold) {
                        // beginFrame errors never reach endFrameChecked(), so
                        // retired resources and after-swap cleanup jobs would
                        // never run again; escalate instead of freezing.
                        failCurrentFrameFatally(QStringLiteral(
                            "Qt Quick Vulkan frame begin failed %1 times in a row")
                            .arg(consecutiveBeginFrameFailures));
                    } else {
                        failCurrentFrame();
                        qCWarning(lcWlRenderer) << "Skipping Vulkan output frame because QRhi beginOffscreenFrame failed"
                                                << "frameResult" << beginResult
                                                << "consecutiveFailures" << consecutiveBeginFrameFailures;
                    }
                }
                finishFrame({});
                return;
    ...
    if (vulkanFrameCompleted) {
        // The frame's command buffer has completed on the GPU. Commit any
        // hardware cursor buffers rendered into it now, so the cursor plane
        // can never scan out content the GPU has not rendered yet.
        for (auto helper : std::as_const(outputs)) {
            if (helper)
                helper->flushPendingHardwareCursor();
        }
    }
    ...
    if (vulkanFrameCompleted) {
        ...
        // All submissions of this completed frame are queue-ordered ahead
        // of the next frame's on the shared VkQueue, so the per-frame alias
        // lists have served their purpose; drop them to keep the alias
        // barriers exact and the scans bounded.
        WRenderHelper::resetVulkanFrameAliasLists(m_renderer);
    }
...
WOutputRenderWindow::~WOutputRenderWindow()
{
    ...
    // Drop frame/needs_frame listeners while outputs are still usable;
    // ~WListenerOwner teardowns cross-object groups automatically.
    d->listenerOwner.reset();
...
void WOutputRenderWindow::attach(WOutputViewport *output)
{
    ...
    if (d->m_renderer) {
        auto wlrOut = newOutput->output();
        if (wlrOut->renderer != d->m_renderer)
            wlr_output_init_render(wlrOut, d->m_allocator, d->m_renderer);
    ...
    if (!containsOutput) {
        ...
        auto *owner = d->listenerOwner.get();
...
void WOutputRenderWindow::init(wlr_renderer *renderer, wlr_allocator *allocator)
{
    Q_D(WOutputRenderWindow);
    ...
    for (auto output : std::as_const(d->outputs)) {
        auto wlrOut = output->output();
        if (wlrOut->renderer != d->m_renderer)
            wlr_output_init_render(wlrOut, d->m_allocator, d->m_renderer);
        Q_EMIT outputViewportInitialized(output->outputViewport());
    }
...
wlr_renderer *WOutputRenderWindow::renderer() const
{
    Q_D(const WOutputRenderWindow);
    return d->m_renderer;
}

wlr_allocator *WOutputRenderWindow::allocator() const
{
    Q_D(const WOutputRenderWindow);
...
{
    Q_D(WOutputRenderWindow);
    d->doRender(nullptr, d->outputs, false, true);
}
