...
void WSurfaceItemPrivate::updateSubsurfaceContainers()
{
    ...
    if (wl_list_empty(&surface->handle()->handle()->current.subsurfaces_below) && belowSubsurfaceContainer) {
        ...
        QObject::connect(belowSubsurfaceContainer,
                         &SubsurfaceContainer::subsurfaceAdded,
                         q,
                         [this](WSurfaceItem *item) {
                             // Container child add/remove is the synchronization point for
                             // the parent live-subsurface registry. New items and reparented
                             // items both enter d->subsurfaces through this path.
    ...
    if (wl_list_empty(&surface->handle()->handle()->current.subsurfaces_above) && aboveSubsurfaceContainer) {
        ...
        QObject::connect(aboveSubsurfaceContainer,
                         &SubsurfaceContainer::subsurfaceAdded,
                         q,
                         [this](WSurfaceItem *item) {
                             // See the below-container connection above.
                             if (!subsurfaces.contains(item))
                                 subsurfaces.append(item);
                         });
        QObject::connect(aboveSubsurfaceContainer,
