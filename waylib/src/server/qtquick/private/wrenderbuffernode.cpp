...
struct WlrAndRhiTexture {
    ...
    bool vulkanOwned = false;
};

class Q_DECL_HIDDEN RhiTextureManager : public DataManager<RhiTextureManager, WlrAndRhiTexture, bool, QRhiTexture::Format, uint32_t, uint64_t, const QSize&, QRhiTexture::Flags>
{
    Q_OBJECT
    ...
    RhiTextureManager(QQuickWindow *owner)
        : DataManager<RhiTextureManager, WlrAndRhiTexture, bool, QRhiTexture::Format,
    ...
    static bool check(WlrAndRhiTexture *texture, bool isVulkan,
                      QRhiTexture::Format,
                      uint32_t drmFormat, uint64_t drmModifier,
                      const QSize &size, QRhiTexture::Flags flags) {
        if (texture->vulkanOwned != isVulkan)
            return false;
        ...
        if (!isVulkan) {
            ...
            if (!wlr_buffer_get_dmabuf(buffer, &attribs)) {
                qCWarning(lcWlRenderBuffer)
                    << "Failed to get dmabuf attributes for texture" << texture
                    << "with buffer" << buffer
                    << ", Can't check texture without dmabuf attributes";
                return false;
            }
            return attribs.format == drmFormat
                && attribs.modifier == drmModifier
    ...
    WlrAndRhiTexture *create(bool isVulkan, QRhiTexture::Format format,
                             uint32_t drmFormat, uint64_t drmModifier,
                             const QSize &size, QRhiTexture::Flags flags) {
        if (!isVulkan) {
...
public:
    ...
    StateFlags changedStates() const override {
        const bool isVulkan = m_item && m_item->window()
            && m_item->window()->graphicsApi() == QSGRendererInterface::Vulkan;
        if (!isVulkan) {
    ...
    RenderingFlags flags() const override {
        ...
        RenderingFlags flags = NoExternalRendering;
        if (!contentNode)
    ...
    void prepare() override {
        ...
        } else if (Q_UNLIKELY(!manager || manager->owner() != window)) {
            const auto oldManager = manager;
            manager = RhiTextureManager::resolveByOwner(manager, window);
            sgTexture()->setTexture(nullptr);
            if (oldManager)
        ...
        else if (Q_UNLIKELY(!rhi || rhi->owner() != window))
...

        {
            ...
            QRhiTexture::Flags textureFlags = isVulkan
                ...
                : QRhiTexture::Flags {};
            texture = manager->resolve(texture, bool(isVulkan), ct->format(), std::move(format),
