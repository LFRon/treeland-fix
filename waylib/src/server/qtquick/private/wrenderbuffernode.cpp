...
public:
    ...
    void prepare() override {
        ...
        if (isVulkan) {
            ...
            if (hasRotation) {
                const QRectF mappedRect = renderMatrix.mapRect(m_rect);
                const QRectF physicalRect(mappedRect.x() * devicePixelRatio,
                                          mappedRect.y() * devicePixelRatio,
                                          mappedRect.width() * devicePixelRatio,
                                          mappedRect.height() * devicePixelRatio);
                vulkanCopySourceRect = physicalRect.normalized().toAlignedRect()
...
private:
    ...
    void renderVulkan(QRhiTexture *currentTexture, QRhiTexture *backdropTexture) {
        ...
        if (hasRotation) {
            ...
            const QPointF sourcePos = renderMatrix.map(m_rect.topLeft());
            const QPointF sourceOffset = vulkanCopySourceRect.topLeft();
            renderData->imageNode->setRect(
                QRectF(-(devicePixelRatio - 1) * sourcePos + sourceOffset,
