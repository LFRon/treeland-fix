...
void wlr_vk_texture_get_image_attribs(struct wlr_texture *texture,
	...
	// For DMA-BUF textures the actual layout follows the acquire/release
	// ownership ping-pong: owned textures are transitioned to
	// SHADER_READ_ONLY_OPTIMAL by the acquire barrier and released back to
	// GENERAL afterwards, so "transitioned" alone would report a stale layout.
	// Pixel (non-DMA-BUF) textures keep the permanent post-upload layout.
	attribs->layout = vk_texture->dmabuf_imported
		? (vk_texture->owned
			? VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL
			: VK_IMAGE_LAYOUT_GENERAL)
		: (vk_texture->transitioned
			? VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL
			: VK_IMAGE_LAYOUT_GENERAL);
...
static bool submit_pending_stage_uploads(struct wlr_vk_renderer *renderer) {
	...
	// to the same queue (see waylib_vk_renderer_set_stage_async_enabled). Fall
...
void waylib_vk_renderer_abort_texture_sync_batch(struct wlr_renderer *wlr_renderer) {
...
bool waylib_vk_renderer_begin_texture_sync_batch(struct wlr_renderer *wlr_renderer) {
	if (wlr_renderer == NULL || !wlr_renderer_is_vk(wlr_renderer)) {
		return false;
	}
...
bool waylib_vk_renderer_flush_texture_sync_batch(struct wlr_renderer *wlr_renderer) {
	if (wlr_renderer == NULL || !wlr_renderer_is_vk(wlr_renderer)) {
		return false;
	}
...
// Returns true when the texture's DMA-BUF was rendered as a color attachment
// by the current frame's command buffer. Such a texture is re-imported for
// sampling in the same command buffer, so its producer writes need an extra
// dependency barrier: an ownership-transfer acquire barrier cannot provide it
// (the source access mask of an acquire operation is ignored and produces no
// availability operation).
static bool is_frame_render_buffer(struct wlr_vk_renderer *renderer,
		struct wlr_buffer *buffer) {
	struct wlr_buffer **entry;
	wl_array_for_each(entry, &renderer->frame_render_buffers) {
		if (*entry == buffer) {
			return true;
		}
	}
...
static void record_frame_render_barrier(struct wlr_vk_renderer *renderer,
		struct wlr_vk_texture *texture, VkCommandBuffer cb) {
	if (texture->buffer == NULL
			|| !is_frame_render_buffer(renderer, texture->buffer)) {
		return;
	}

	VkImageMemoryBarrier barrier = {
		.sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER,
		// Equal queue families and equal layouts: with synchronization2 enabled
		// the layout values are ignored and the image contents are preserved,
		// so this barrier only extends the producer's color-attachment writes
		// to the sampling without touching layout or ownership state.
	...
	if (renderer->texture_barrier_batch_active
		...
		VkImageMemoryBarrier *slot = wl_array_add(
			&renderer->frame_render_barriers, sizeof(barrier));
		if (slot != NULL) {
			*slot = barrier;
			deferred = true;
		} else {
			wlr_log_errno(WLR_ERROR,
				"Failed to queue Vulkan frame-producer barrier");
		}
	}
	if (!deferred) {
	...
	static bool logged_frame_producer_barrier;
	if (!logged_frame_producer_barrier) {
		logged_frame_producer_barrier = true;
		wlr_log(WLR_DEBUG, "Vulkan texture sampling: extending same-frame "
			"color-attachment writes to the sampling of a re-imported "
			"DMA-BUF texture");
	}
}
...
bool waylib_vk_renderer_prepare_texture_for_sampling(struct wlr_renderer *wlr_renderer,
	...
	if (texture->dmabuf_imported && !texture->owned) {
		...
		record_frame_render_barrier(renderer, texture, cb);
...
bool waylib_vk_renderer_finish_texture_sampling(struct wlr_renderer *wlr_renderer,
		struct wlr_texture *wlr_texture, VkCommandBuffer cb) {
	if (wlr_renderer == NULL || wlr_texture == NULL || cb == VK_NULL_HANDLE) {
		return false;
	}
...
bool waylib_vk_renderer_begin_texture_barrier_batch(struct wlr_renderer *wlr_renderer,
		bool release) {
	if (wlr_renderer == NULL || !wlr_renderer_is_vk(wlr_renderer)) {
		return false;
	}
...
bool waylib_vk_renderer_flush_texture_barrier_batch(struct wlr_renderer *wlr_renderer,
		VkCommandBuffer cb) {
	if (wlr_renderer == NULL || !wlr_renderer_is_vk(wlr_renderer)) {
		return false;
	}
	...
	// Same-frame producer dependencies are only queued during the acquire
	// (pre-draw) phase and must be recorded after the ownership-transfer
	// acquire barriers above.
	const size_t frame_count = renderer->frame_render_barriers.size /
		sizeof(VkImageMemoryBarrier);
	if (cb != VK_NULL_HANDLE && !release && frame_count > 0) {
		vkCmdPipelineBarrier(cb,
			VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT,
			VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT, 0,
			0, NULL, 0, NULL, (uint32_t)frame_count,
			renderer->frame_render_barriers.data);
	}

	barriers->size = 0;
	renderer->frame_render_barriers.size = 0;
	renderer->texture_barrier_batch_active = false;
	renderer->texture_barrier_batch_release = false;
	return true;
}

void waylib_vk_renderer_abort_texture_barrier_batch(struct wlr_renderer *wlr_renderer) {
	if (wlr_renderer == NULL || !wlr_renderer_is_vk(wlr_renderer)) {
		return;
	}
