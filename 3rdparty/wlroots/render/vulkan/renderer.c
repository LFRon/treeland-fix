...
bool waylib_vk_renderer_record_render_buffer_release(struct wlr_renderer *wlr_renderer,
	...
	// Remember the buffer as a color-attachment producer of the current
	// frame's command buffer. A sampled texture re-imported from this DMA-BUF
	// needs an extra dependency barrier, because an ownership-transfer acquire
	// barrier cannot order these writes (its source access mask is ignored).
	bool known_producer = false;
	struct wlr_buffer **known_entry;
	wl_array_for_each(known_entry, &renderer->frame_render_buffers) {
		if (*known_entry == wlr_buffer) {
			known_producer = true;
			break;
		}
	}
	if (!known_producer) {
		struct wlr_buffer **slot = wl_array_add(
			&renderer->frame_render_buffers, sizeof(*slot));
		if (slot != NULL) {
			*slot = wlr_buffer;
		} else {
...
static void vulkan_destroy(struct wlr_renderer *wlr_renderer) {
	...
	wl_array_release(&renderer->frame_render_barriers);
...
struct wlr_renderer *vulkan_renderer_create_for_device(struct wlr_vk_device *dev) {
	...
	wl_array_init(&renderer->frame_render_barriers);
