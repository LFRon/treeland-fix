...
struct wlr_vk_renderer {
	...
	// Plain same-frame-producer dependency barriers (VkImageMemoryBarrier) for
	// textures whose DMA-BUF was rendered as a color attachment by the current
	// frame's command buffer and is re-imported for sampling. An acquire
	// barrier cannot order those writes (its source access mask is ignored),
	// so an equal-layout, equal-queue-family barrier extends them to the
	// sampling instead.
	struct wl_array frame_render_barriers;
